    ...
    board.addEventListener('click', (e) => {
        ...
        if (game.selectedPiece) {
            ...
            if (game.turn === targetColor) {
                const selectedCell = document.getElementById(`${selectedTop}-${selectedLeft}-${game.selectedPiece.name}`);
                selectedCell?.classList.toggle('selected');
                game.legalMoves.forEach(({ top, left }) => {
                    const { name } = game.pieces.find(({ top: t, left: l }) => t === top && l === left);
                    const cell = document.getElementById(`${top}-${left}` + (name ? `-${name}` : ''));
                    if (cell) {
            ...
            if (true) {
                const { left: selectedLeft, top: selectedTop, name: selectedName, } = game.selectedPiece;
                const selectedPiece = game.pieces.find(({ top, left }) => top === selectedTop && left === selectedLeft);
                const selectedElement = document.getElementById(`${selectedTop}-${selectedLeft}-${selectedPiece?.name}`);
                const pieceElement = document.getElementById(`img${selectedTop}-${selectedLeft}-${selectedName}`);
                const targetPieceElement = document.getElementById(`img${top}-${left}-${piece}`);
