...
export class Game {
  ...
  filterLegalMovesThatPutKingInCheck() {
    const filteredLegalMoves = this.legalMoves.filter(({ top, left }) => {
      ...
      const game = new Game();
      const allPossibleOpponentNextMoves: { top: number; left: number }[] = [];
      game.loadGame(JSON.parse(JSON.stringify(this)));
      game.movePiece(
        this.selectedPiece!.top,
        this.selectedPiece!.left,
        top,
        left
      );
      ...
      if (
        allPossibleOpponentNextMoves.some((move) => {
          const previousBoard = game.history[game.history.length - 2];

          if (move.top === 7) {
            if (
              game.board[7][6].piece === 'white_king' &&
            ...
            if (
              game.board[7][2].piece === 'white_king' &&
          ...
          if (move.top === 0) {
            if (
              game.board[0][6].piece === 'black_king' &&
            ...
            if (
              game.board[0][2].piece === 'black_king' &&
  ...
  movePiece(
    ...
    if (
      selectedPieceName?.includes('pawn') &&
      !targetPieceColor &&
    ...
    if (
      this.selectedPiece?.piece.includes('king') &&
      (targetTop === 0 || targetTop === 7) &&
